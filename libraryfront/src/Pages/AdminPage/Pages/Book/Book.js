...
export default function Book() {
    ...
    const handleDelete = async(id) => {
        ...
        const response = await axios.delete(`http://localhost:5142/api/Book/${id}`, {headers : {
    ...
    const handleUpdate = async(b) => {
        const token = localStorage.getItem('token');
        const refreshToken = localStorage.getItem('refresh');
        const response = await axios.get(`http://localhost:5142/api/Book/${b.id}`, {headers : {
            Authorization: `Bearer ${token}`,
            RefreshToken: refreshToken
        }});
  ...
  return (
    <div className='ml-80'>
        <div className='my-3 p-4  mr-5 grid gap-7 relative rounded-xl bg-white text-gray-700 shadow-lg border border-gray-100 overflow-hidden'>
            ...
            type="button"
            onClick={() => setCreateModal(true)}>Create Book</button>
            </div>
            <div className='flex flex-col items-center '>
        <div className='grid grid-cols-4 gap-10'>
            {books && currentBooks.map((book) => <SingleBook book={book} Delete={handleDelete} handleUpdate={handleUpdate}/>)}
            </div>
            {books && <Paginate 
